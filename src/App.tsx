...
const Skills = () => {
  ...
  const TechLogo = ({ name, children }: { name: string; children: React.ReactNode }) => (
    <div className="group relative">
      <div className="w-16 h-16 sm:w-14 sm:h-14 lg:w-16 lg:h-16 flex items-center justify-center bg-gray-800 rounded-xl border border-gray-700 hover:border-blue-400 transition-all duration-300 hover:scale-110 hover:bg-gray-700">
        {children}
      </div>
      <span className="absolute -bottom-10 sm:-bottom-8 left-1/2 transform -translate-x-1/2 text-sm text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap">
        {name}
      </span>
    </div>
  );
